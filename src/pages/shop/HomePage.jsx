      ...
      <section className="py-10 sm:py-14 bg-slate-50 border-y border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 overflow-hidden">
            <div className="brands-track flex items-center gap-4 py-4" role="list" aria-label="Brand logos">
              {[...brands, ...brands].map((brand, idx) => (
                <Link key={idx} to={`/shop?search=${encodeURIComponent(brand.name)}`} className="brand-marquee-item relative flex-shrink-0 w-28 sm:w-32 md:w-36 lg:w-40 p-3 flex flex-col items-center justify-center bg-white rounded-xl border border-slate-200 shadow-sm">
                  {brand.verified && (
                  ...
                  <img src={brand.iconUrl} alt={brand.name} loading="lazy" decoding="async" className={`w-12 h-12 sm:w-14 sm:h-14 object-contain ${brand.invertDark ? 'dark:invert' : ''}`} />
